...
object Advent7 {
  ...
  def readCircuits(input:String): Map[String, Circuit] = {
    val regex = "(.+) \\((\\d+)\\)( -> (.*))?".r
    val entries:mutable.Map[String, Circuit] = new mutable.HashMap[String, Circuit]
    input.split('\n').foreach { circuitDefinition =>
      regex.findFirstMatchIn(circuitDefinition).foreach { defMatch =>
        val name = defMatch.group(1)
        val weight = defMatch.group(2).toInt
        val children = Option(defMatch.group(4)).map(_.split(',').map(_.trim).toList).getOrElse(Nil)
        val newCircuit = entries.get(name)
          .map(_.copy(weight = weight, children = children))
          .getOrElse(Circuit(None, name, weight, children))
        entries.put(name, newCircuit)

        children.foreach { childName =>
          entries.get(childName) match {
            case Some(child) => entries.put(child.name, child.copy(parent = Some(name)))
            case None => entries.put(childName, Circuit(Some(name), childName, 0, List()))
          }
        }
      }
    }
