...
case object NEQ extends CMP

object Advent8 {
  type Memory = Map[String, Int]

  case class Condition(register:String, comparison: CMP, value: Int) {
    def test(memory: Memory): Boolean = {
      val registerValue = memory.getOrElse(register, 0)

      comparison match {
        case GE => registerValue >= value
        case LE => registerValue <= value
        case GT => registerValue > value
        case LT => registerValue < value
        case EQ => registerValue == value
        case NEQ => registerValue != value
      }
    }
  }

  case class Instruction(register:String, operation: Operation, amount: Int, condition: Condition) {
    def perform(memory: Memory): Memory = {
      if (condition.test(memory)) {
        val plusValue = operation match {
          case INC => amount
          case DEC => -amount
        }

        val newValue = memory.getOrElse(register, 0) + plusValue
        memory + (register -> newValue)
      } else memory
    }
  }
  ...
  def parse(s:String):Option[Instruction] = {
    ...
    if (tokens.length == 7) {
      ...
      val operation = if (tokens(1) == "inc") INC else DEC
      val amount = tokens(2).toInt
      val cmpRegister = tokens(4)
      val cmpOp = tokens(5) match {
        case "<" => LT
        case ">" => GT
        case ">=" => GE
        case "<=" => LE
      ...
      val cmpAmount = tokens(6).toInt

      Some(Instruction(register, operation, amount, Condition(cmpRegister, cmpOp, cmpAmount)))
    } else None
  }
  ...
  private val TestInstructions = TestInput.split('\n').flatMap(parse)

  private def run(memory: Memory, instructions: Iterable[Instruction]) = instructions.scanLeft(memory) { case (mem, instruction) =>
    instruction.perform(mem)
  }



  def main(args:Array[String]):Unit = {
    val allStates = run(Map(), TestInstructions)
    val maxValues = allStates.filterNot(_.isEmpty).map(_.values.max)

    val solution1 = maxValues.last
    val solution2 = maxValues.max
